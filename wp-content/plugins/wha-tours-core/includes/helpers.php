<?php
/**
 * Public helpers used by the WHA Tour Poster theme.
 *
...
if ( ! function_exists( 'wha_tours_core_get_meta' ) ) {
	 ...
	 * }
	 */
	function wha_tours_core_get_meta( $post_id = 0 ) {
		...
		$route = get_post_meta( $post_id, '_wha_tour_route', true );
		$dates = get_post_meta( $post_id, '_wha_tour_dates', true );

		return array(
			'days'           => absint( get_post_meta( $post_id, '_wha_tour_days', true ) ),
			'transport'      => wha_tours_core_sanitize_transport( get_post_meta( $post_id, '_wha_tour_transport', true ) ),
			'price'          => (string) get_post_meta( $post_id, '_wha_tour_price', true ),
			'route'          => wha_tours_core_sanitize_route( $route ),
			'dates'          => wha_tours_core_sanitize_dates( $dates ),
			'hit'            => '' !== (string) get_post_meta( $post_id, '_wha_tour_hit', true ),
			'next_departure' => (string) get_post_meta( $post_id, '_wha_tour_next_departure', true ),
		);
	}
}

if ( ! function_exists( 'wha_tours_core_get_tours' ) ) {
	/**
	 * Query tours.
	 *
	 * Tours are ordered by the computed `_wha_tour_next_departure` meta, soonest
	 ...
	 * @return WP_Query
	 */
	function wha_tours_core_get_tours( array $args = array() ) {
		$args = wp_parse_args(
			$args,
			array(
				'season'   => '',
		...
		if ( ! empty( $args['upcoming'] ) ) {
			$meta_query['wha_tour_next_departure'] = array(
				'key'     => '_wha_tour_next_departure',
				'value'   => current_time( 'Y-m-d' ),
				'compare' => '>=',
				'type'    => 'DATE',
			);

			$query_args['orderby'] = array( 'wha_tour_next_departure' => $order );
			...
			$query_args['wha_tours_core_nulls_last'] = $order;
		...
		/**
		 * Filter the arguments used by `wha_tours_core_get_tours()`.
		 *
		 * @param array $query_args WP_Query arguments.
		 * @param array $args       Helper arguments.
		 */
		$query_args = apply_filters( 'wha_tours_core_get_tours_args', $query_args, $args );

		return new WP_Query( $query_args );
	}
}

if ( ! function_exists( 'wha_tours_core_get_term_image_id' ) ) {
	/**
	 ...
	 */
	function wha_tours_core_get_term_image_id( $term_id ) {
		$term_id = (int) $term_id;
		...
		return absint( get_term_meta( $term_id, '_wha_tour_image_id', true ) );
	}
}

if ( ! function_exists( 'wha_tours_core_next_date' ) ) {
	/**
	 ...
	 * }
	 */
	function wha_tours_core_next_date( $dates ) {
		$dates = wha_tours_core_sanitize_dates( $dates );
		...
		$found['label'] = wha_tours_core_date_range_label( $found['start'], $found['end'] );

		return $found;
	}
}

if ( ! function_exists( 'wha_tours_core_date_range_label' ) ) {
	/**
	 ...
	 * @return string
	 */
	function wha_tours_core_date_range_label( $start, $end = '' ) {
		$start = wha_tours_core_sanitize_date( $start );
		$end   = wha_tours_core_sanitize_date( $end );
		...
		return sprintf( _x( '%1$s – %2$s', 'departure date range', 'wha-tours-core' ), $start_label, $end_label );
	}
}

if ( ! function_exists( 'wha_tours_core_format_dates' ) ) {
	/**
	 ...
	 */
	function wha_tours_core_format_dates( array $dates ) {
		$dates = wha_tours_core_sanitize_dates( $dates );
		...
		$next = wha_tours_core_next_date( $dates );
		...
		if ( $rest > 0 ) {
			...
			$label = sprintf( _x( '%1$s +%2$d', 'departure dates summary', 'wha-tours-core' ), $label, $rest );
		}
...
if ( ! function_exists( 'wha_tours_core_transport_label' ) ) {
	/**
	 ...
	 */
	function wha_tours_core_transport_label( $key ) {
		$labels = array(
			'bus'   => __( 'Bus', 'wha-tours-core' ),
			'plane' => __( 'Plane', 'wha-tours-core' ),
			'train' => __( 'Train', 'wha-tours-core' ),
			'mixed' => __( 'Combined', 'wha-tours-core' ),
		);

		$key = wha_tours_core_sanitize_transport( $key );

		return isset( $labels[ $key ] ) ? $labels[ $key ] : '';
	}
}

if ( ! function_exists( 'wha_tours_core_transport_icon' ) ) {
	/**
	 ...
	 */
	function wha_tours_core_transport_icon( $key ) {
		$icons = array(
			'bus'   => 'bus',
			'plane' => 'plane',
			'train' => 'bus',
...
if ( ! function_exists( 'wha_tours_core_category_color' ) ) {
	/**
	 ...
	 */
	function wha_tours_core_category_color( $term ) {
		if ( is_numeric( $term ) ) {
		...
		$color = wha_tours_core_sanitize_color( get_term_meta( $term->term_id, '_wha_tour_color', true ) );
		...
		$colors = wha_tours_core_get_colors();

		return $colors[ absint( crc32( $slug ) ) % count( $colors ) ];
	}
}

if ( ! function_exists( 'wha_tours_core_price_label' ) ) {
	/**
	 ...
	 * @return string
	 */
	function wha_tours_core_price_label( $price ) {
		$price = is_scalar( $price ) ? trim( (string) $price ) : '';

		return '' !== $price ? $price : __( 'on request', 'wha-tours-core' );
	}
}
 ...
 * @return string[]
 */
function wha_tours_core_next_departure_clauses( $clauses, $query ) {
	global $wpdb;
	...
	$order = $query->get( 'wha_tours_core_nulls_last' );
	...
	$clauses['join'] .= $wpdb->prepare(
		" LEFT JOIN {$wpdb->postmeta} AS wha_nd ON ( {$wpdb->posts}.ID = wha_nd.post_id AND wha_nd.meta_key = %s )",
		'_wha_tour_next_departure'
	);

	$clauses['orderby'] = "CASE WHEN wha_nd.meta_value IS NULL OR wha_nd.meta_value = '' THEN 1 ELSE 0 END ASC, wha_nd.meta_value {$order}, {$wpdb->posts}.post_date DESC";

	return $clauses;
}
add_filter( 'posts_clauses', 'wha_tours_core_next_departure_clauses', 10, 2 );

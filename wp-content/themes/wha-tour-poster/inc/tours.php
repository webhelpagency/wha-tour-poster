<?php
/**
 * Theme side wrappers around the WHA Tours Core plugin.
 *
...
if ( ! function_exists( 'wha_tour_poster_has_core' ) ) :
	/**
	 * Whether the WHA Tours Core plugin is active and ready.
	 *
	 * @return bool True when the plugin helpers and the `tour` post type exist.
	 */
	function wha_tour_poster_has_core() {
		return function_exists( 'wha_tours_core_get_tours' ) && post_type_exists( 'tour' );
	}
endif;

if ( ! function_exists( 'wha_tour_poster_archive_link' ) ) :
	/**
	 * URL of the programme catalogue.
	 *
	 * @return string Tour archive URL, the blog page URL as a fallback.
	 */
	function wha_tour_poster_archive_link() {
...
if ( ! function_exists( 'wha_tour_poster_term_color' ) ) :
	/**
	 ...
	 */
	function wha_tour_poster_term_color( $term ) {
		...
		if ( 'tour_category' === $term->taxonomy && function_exists( 'wha_tours_core_category_color' ) ) {
			return wha_tours_core_category_color( $term );
		}
...
if ( ! function_exists( 'wha_tour_poster_get_post_term' ) ) :
	/**
	 ...
	 */
	function wha_tour_poster_get_post_term( $post_id ) {
		$taxonomy = 'category';
...
if ( ! function_exists( 'wha_tour_poster_get_home_categories' ) ) :
	/**
	 ...
	 */
	function wha_tour_poster_get_home_categories() {
		$taxonomy = wha_tour_poster_has_core() && taxonomy_exists( 'tour_category' ) ? 'tour_category' : 'category';
		...
		foreach ( $terms as $term ) {
			...
			if ( 'tour_category' === $taxonomy ) {
				if ( function_exists( 'wha_tours_core_get_term_image_id' ) ) {
					$image_id = wha_tours_core_get_term_image_id( $term->term_id );
				}

				$short = trim( (string) get_term_meta( $term->term_id, '_wha_tour_short_desc', true ) );
			...
			$categories[] = array(
				...
				'color'       => wha_tour_poster_term_color( $term ),
			);
		}
...
if ( ! function_exists( 'wha_tour_poster_get_seasons' ) ) :
	/**
	 * Season terms in the order set by the plugin.
	 *
	 * @return WP_Term[] Terms, empty without the companion plugin.
	 */
	function wha_tour_poster_get_seasons() {
		if ( ! wha_tour_poster_has_core() || ! taxonomy_exists( 'tour_season' ) ) {
			return array();
		}
...
if ( ! function_exists( 'wha_tour_poster_get_season_tours' ) ) :
	/**
	 * Tours of one season.
	 *
	 ...
	 */
	function wha_tour_poster_get_season_tours( $slug ) {
		if ( ! wha_tour_poster_has_core() ) {
			return new WP_Query( array( 'post__in' => array( 0 ) ) );
		}

		return wha_tours_core_get_tours(
			array(
...
if ( ! function_exists( 'wha_tour_poster_get_hit_tours' ) ) :
	/**
	 ...
	 */
	function wha_tour_poster_get_hit_tours() {
		if ( wha_tour_poster_has_core() ) {
			return wha_tours_core_get_tours(
				array(
					'hit'   => true,
...
if ( ! function_exists( 'wha_tour_poster_current_season_slug' ) ) :
	/**
	 ...
	 */
	function wha_tour_poster_current_season_slug() {
...
if ( ! function_exists( 'wha_tour_poster_pre_get_posts' ) ) :
	/**
	 ...
	 * ordering `wha_tours_core_get_tours()` uses, so that the archives and the
	 ...
	 */
	function wha_tour_poster_pre_get_posts( $query ) {
		...
		if ( $query->is_search() ) {
			...
			if ( ! $query->get( 'post_type' ) ) {
				$wha_tour_poster_types = get_post_types( array( 'exclude_from_search' => false ) );

				// Media is searchable by default, but an attachment among the
				// programmes and posts is never a useful result here.
				unset( $wha_tour_poster_types['attachment'] );

				$wha_tour_poster_types['tour'] = 'tour';

				$query->set( 'post_type', array_values( $wha_tour_poster_types ) );
			}
		...
		// `_wha_tour_next_departure` meta through `posts_clauses`: soonest
		// departure first, tours without a departure date last.
		$query->set( 'wha_tours_core_nulls_last', 'ASC' );
		$query->set( 'orderby', 'date' );
		$query->set( 'order', 'DESC' );
	}
endif;
add_action( 'pre_get_posts', 'wha_tour_poster_pre_get_posts' );
